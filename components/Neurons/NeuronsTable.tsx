...
export const neuronsTableColumns = [
  {
    Header: "ID",
    accessor: "id",
  ...
  },
  {
    ...
    Cell: ({ value, row }) => (
      <BalanceLabel
        value={
          row.original.account && row.original.state === NeuronState.Dissolved
...
const NeuronsTable = ({ name }: { name: string }) => {
  const initialSort = useMemo(() => [{ id: "stake", desc: true }], []);
